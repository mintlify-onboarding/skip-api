...
description: "This document explains how to use Skip API and/or the skip-router SDK to construct SVM transactions."
---

## Intro

* When a user needs to transfer or swap from an SVM chain (e.g. Solana), the Skip API will return an `SvmTx` type for the developer to pass to the user for signing
...
[skip-router](https://www.npmjs.com/package/@skip-router/core) takes care of all of the complexity of signing the transaction that gets returned in `SvmTx.tx`. 
